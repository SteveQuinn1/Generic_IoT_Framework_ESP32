...
// Requires WiFi Library found here:                                 https://github.com/espressif/arduino-esp32/tree/master/libraries/WiFi
// Requires Arduino IDE support for ESP32 found here:                https://github.com/espressif/arduino-esp32/blob/master/docs/arduino-ide/boards_manager.md
// Requires SPIFFS file system support found in ESP32 Core or here : https://github.com/espressif/arduino-esp32/tree/master/libraries/SPIFFS
// Requires WebServer Library found here:                            https://github.com/espressif/arduino-esp32/tree/master/libraries/WebServer
// Requires ESPmDNS Library found here:                              https://github.com/espressif/arduino-esp32/tree/master/libraries/ESPmDNS
// 
// ESP32 generic IoT framework for connecting to WiFi and Broker has one led output (if fitted to a given dev board)
//
// Compiled on Arduino IDE v1.8.5, Boards Manager : ESP32 by espressif systems v1.0.2
//
// Also reference the following : https://github.com/me-no-dev/arduino-esp32fs-plugin/releases/ v1.0
//
// Files -> Preferences -> Additional Boards Manager URLs: https://dl.espressif.com/dl/package_esp32_index.json
// 
//
// Maintenance History
// 18/04/19 : 1   Adapted 'GenericIoTFrameWork2_1' ESP8266 variant for ESP32 framework.
//
//
// Start up sequence
// -----------------
...
// for which you can either enter the IP address 192.168.4.1 or nDNSHostName + '.local'.
...
// Any files residing here will be uploaded to the SPIFFS area on the ESP32 when Tools->ESP32 Sketch Data Upload is selected. 
// Here there are two text files named 'secvals.txt' and 'index.htm' + some other crap such that a little 'IoT' icon appears in the browser address bar.
//
...
String clientName;
const char* THIS_GENERIC_DEVICE = "esp32";
String swVersion;
...
void setup() {
  ...
  // This is enabled or the TX/RX port will require 10K pull ups to stop oscillations of the I/Ps which makes the ESP8266-01 pull more current. It is assumed the same is true for the ESP32.
  //#ifdef DEBUG_GENERAL
  Serial.begin(115200);
  delay(100);
  //#endif
...
void loop(){
  ...
  switch (eGENIOTSTATE_STATE) {
    case eGENIOTSTATE_INIT : //
           ...
           WiFi.encryptionType(WIFI_AUTH_WEP);
    ...
    case eGENIOTSTATE_ACTIVE : // Run as a WiFi client in active mode
           ...
           // Delay to allow ESP32 WIFI functions to run
           //yield();
           delay(10); 
           break;
  }
